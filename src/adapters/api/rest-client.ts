...
export interface RestExecutionResult {
  response: unknown;
  captures: Record<string, unknown>;
  evidencePayload: ApiStepEvidence;
  apiEvidence: ApiStepEvidence;
}

export class RestClient {
  private readonly client: AxiosInstance;

  constructor(private readonly env: EnvironmentConfig, private readonly rootDir: string) {
  ...
  async execute(
    ...
    requestOverride?: ApiRequestSpec,
    assertions: ApiAssertion[] = [],
    explicitCaptures: Record<string, string> = {},
    options: RestExecutionOptions = {}
  ): Promise<RestExecutionResult> {
    if (!this.env.apiBaseUrl) throw new Error("ADFINEM_API_BASE_URL is required for API execution.");
    if (operation.type !== "rest") throw new Error(`Operation type '${operation.type}' is not supported by RestClient.`);
    const request = withGeneratedHeaders(renderRequestSpec(mergeApiRequest(operation, requestOverride), input));
    if (!request.method || !request.path) throw new Error("REST operation must define method and path.");
    ...
    const apiEvidence: ApiStepEvidence = {
  ...
  private async requestWithRetry(operation: ApiOperationEntry, request: ApiRequestSpec, body: unknown): Promise<AxiosResponse<unknown>> {
      ...
      try {
        ...
        const response = await this.client.request<unknown>({
          ...
          params: method === "GET" || method === "DELETE" || method === "HEAD"
            ? { ...(request.query ?? {}), ...(body && typeof body === "object" && !Array.isArray(body) ? body as Record<string, unknown> : {}) }
            : request.query,
          validateStatus: () => true
        });
        (response as AxiosResponse<unknown> & { durationMs?: number }).durationMs = Date.now() - started;
        ...
        lastError = error;
        if (!allowRetry || !isTransientError(error) || attempt >= maxAttempts) throw error;
        await sleep(DEFAULT_RETRY_DELAY_MS * (attempt + 1));
      }
    }
  ...
  private async loadTemplate(relativePath: string, input: Record<string, unknown>): Promise<unknown> {
    ...
    const rendered = raw.replace(/\$\{([A-Za-z0-9_.-]+)\}/g, (_match, name: string) => {
      const value = input[name];
      if (value === undefined || value === null) throw new Error(`API template variable '${name}' was not supplied.`);
...
function requestEvidenceFor(request: ApiRequestSpec, body: unknown): ApiRequestEvidence {
  return {
...
function normalizeHttpResponse(response: AxiosResponse<unknown>): ApiResponseEvidence {
  ...
  return {
    ...
    bodyJson,
    contentType,
    durationMs: (response as AxiosResponse<unknown> & { durationMs?: number }).durationMs ?? 0,
    sizeBytes,
    bodyTruncated,
    bodyPreviewKind
  };
...
function responseBodyPreview(data: unknown, contentType: string): Pick<ApiResponseEvidence, "body" | "bodyText" | "bodyJson" | "sizeBytes" | "bodyTruncated" | "bodyPreviewKind"> {
...
function isBinaryContentType(contentType: string): boolean {
  ...
  if (normalized.includes("json") || normalized.startsWith("text/") || normalized.includes("xml") || normalized.includes("html")) return false;
  return true;
}
...
function normalizeAcceptStatuses(request: ApiRequestSpec & { acceptedStatuses?: number[] }): number[] {
...
function isStatusAccepted(status: number, acceptStatuses: number[]): boolean {
  return acceptStatuses.length > 0 ? acceptStatuses.includes(status) : status >= 200 && status < 300;
}

function failureReasonFor(status: number, acceptStatuses: number[], assertions: ApiAssertionResult[], captureFailures: CaptureResult[]): string {
  ...
  if (failedAssertion) return failedAssertion.message ?? "One or more response assertions failed.";
  const failedCapture = captureFailures[0];
  if (failedCapture) return failedCapture.message ?? `Required capture '${failedCapture.name}' was not extracted.`;
...
function isTransientError(error: unknown): boolean {
  ...
  if (candidate.response?.status && TRANSIENT_STATUS_CODES.has(candidate.response.status)) return true;
...
function requestBody(request: ApiRequestSpec, input: Record<string, unknown>): unknown {
  ...
  if (request.body !== undefined) return renderValue(request.body, input);
  return input;
}

function renderRequestSpec(request: ApiRequestSpec, input: Record<string, unknown>): ApiRequestSpec {
  return renderValue(request, input) as ApiRequestSpec;
}

function renderValue(value: unknown, input: Record<string, unknown>): unknown {
  if (typeof value === "string") return renderTemplateString(value, input);
...
function renderTemplateString(value: string, input: Record<string, unknown>): string {
  return value
    .replace(/\$\{([A-Za-z0-9_.-]+)\}/g, (_match, name: string) => renderedInputValue(input, name))
    .replace(/\{\{([A-Za-z0-9_$.-]+)\}\}/g, (_match, name: string) => renderedInputValue(input, name));
}

function renderedInputValue(input: Record<string, unknown>, name: string): string {
  const generated = postmanGeneratedValue(name);
  if (generated !== undefined) return generated;
  const value = input[name];
  if (value === undefined || value === null) throw new Error(`API request variable '${name}' was not supplied.`);
  return typeof value === "string" ? value : JSON.stringify(value);
}

function postmanGeneratedValue(name: string): string | undefined {
  if (name === "$guid" || name === "$randomUUID") return randomUUID();
...
export function withGeneratedHeaders(request: ApiRequestSpec): ApiRequestSpec {
  const headers = {
...
function generatedHeadersFor(request: ApiRequestSpec): Record<string, string> {
  const headers: Record<string, string> = { Accept: "*/*", "Cache-Control": "no-cache" };
...
export function requestPathForBase(path: string, baseUrl: string | undefined): string {
  ...
  try {
    ...
    if (lowerPath.startsWith(`${lowerBase}/`)) return path.slice(basePath.length) || "/";
    return path;
  } catch {
    return path;
  }
...
export function evaluateApiAssertions(response: AxiosResponse<unknown>, assertions: ApiAssertion[]): ApiAssertionResult[] {
  return assertions.map((assertion) => evaluateApiAssertion(response, assertion));
}

function evaluateApiAssertion(response: AxiosResponse<unknown>, assertion: ApiAssertion): ApiAssertionResult {
  try {
  ...
  return { assertion, passed: false, message: `Unsupported assertion type '${(assertion as { type?: string }).type ?? "unknown"}'.` };
}

export function assertApiResponse(response: AxiosResponse<unknown>, assertions: ApiAssertion[]): void {
  const failed = evaluateApiAssertions(response, assertions).find((result) => !result.passed);
  if (failed) throw new Error(failed.message ?? "API response assertion failed.");
}

function jsonPathValues(data: unknown, path: string): unknown[] {
  return JSONPath({ path, json: data as object | unknown[] | string | number | boolean | null, wrap: true }) as unknown as unknown[];
}

function headerValue(response: AxiosResponse<unknown>, name: string): string | undefined {
  const normalized = name.toLowerCase();
  for (const [header, value] of Object.entries(response.headers ?? {})) {
    if (header.toLowerCase() !== normalized) continue;
...
export function extractJsonCaptures(data: unknown, captureSpec: Record<string, string>): Record<string, unknown> {
  return captureResultsToRecord(extractJsonCaptureResults(data, {}, captureSpec));
}

export function extractJsonCaptureResults(data: unknown, catalogCaptures: Record<string, string>, explicitCaptures: Record<string, string>): CaptureResult[] {
  ...
  for (const [name, rawExpr] of Object.entries({ ...catalogCaptures, ...explicitCaptures })) {
    ...
    try {
      const result = JSONPath({ path: parsed.path, json: data as object | unknown[] | string | number | boolean | null, wrap: true }) as unknown as unknown[];
      if (result.length === 0) {
        results.push({
          name,
          expression: rawExpr,
...
function skippedCaptureResults(catalogCaptures: Record<string, string>, explicitCaptures: Record<string, string>, message: string): CaptureResult[] {
  const explicit = new Set(Object.keys(explicitCaptures));
...
function captureResultsToRecord(results: CaptureResult[]): Record<string, unknown> {
  return Object.fromEntries(results
    .filter((result) => result.status === "extracted")
    .map((result) => [result.name, result.value]));
}

function parseCaptureExpression(expression: string): { path: string; mode: "auto" | "array" | "scalar"; required: boolean } {
  ...
  if (trimmed.startsWith("array:")) return { path: trimmed.slice("array:".length).trim(), mode: "array", required };
  if (trimmed.startsWith("scalar:")) return { path: trimmed.slice("scalar:".length).trim(), mode: "scalar", required };
  return { path: trimmed, mode: "auto", required };
}

function responseFieldHint(data: unknown): string {
  const hints = collectJsonPathHints(data, "$", 0).slice(0, 10);
  return hints.length ? ` Available response fields include: ${hints.join(", ")}.` : "";
}

function collectJsonPathHints(value: unknown, prefix: string, depth: number): string[] {
