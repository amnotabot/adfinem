...
import type { RunResult, StepResult } from "../dsl/types.js";
...
function countByStatus(steps: StepResult[]): { passed: number; failed: number; skipped: number } {
  let passed = 0;
  let failed = 0;
  let skipped = 0;
...
function stepDurationMs(step: StepResult): number | undefined {
...
function formatDuration(ms: number | undefined): string {
  if (ms === undefined) return "&mdash;";
  if (ms < 1000) return `${ms} ms`;
  return `${(ms / 1000).toFixed(2)} s`;
}

function renderEvidenceLinks(step: StepResult, evidenceDir: string): string {
  if (!step.evidence?.length) return "&mdash;";
...
function relativeOrAbsolute(target: string, base: string): string {
  try {
