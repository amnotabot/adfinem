...
import type { FlowActionNode, FlowFile, FlowNode } from "./types.js";
...
export function normalizeFlowCatalogParams(flow: FlowFile, catalogs: Catalogs): FlowFile {
  return {
    ...flow,
...
function normalizeNode(node: FlowNode, flow: FlowFile, catalogs: Catalogs): FlowNode {
...
function normalizeAction(node: FlowActionNode, flow: FlowFile, catalogs: Catalogs): FlowActionNode {
  if (node.type === "db_query" || node.type === "db_assert" || node.type === "db_execute") {
    const specs = catalogs.queries[node.query]?.params;
    if (!specs || Object.keys(specs).length === 0) return node;
    return {
      ...node,
...
function normalizeNamedParams(
  ...
  specs: Record<string, CatalogParam>
