import type { CatalogArg, CatalogParam, Catalogs } from "../dsl/types.js";
...
import type { FlowActionNode, FlowFile, FlowNode, FlowValidationResult } from "./types.js";
import { validateScenarioReferences } from "../dsl/validator.js";

export async function validateFlow(flow: FlowFile, catalogs: Catalogs, rootDir: string, environment = flow.environment): Promise<FlowValidationResult> {
  ...
  for (const node of effectiveFlow.nodes) {
    ...
    if (node.type === "api_operation") {
      ...
      for (const postAction of node.postActions ?? []) {
        ...
        if (postAction.disabled) warnings.push(`Flow action '${postAction.id}' is disabled and will be skipped.`);
  ...
  if (compiled.scenario.steps.length > 0) {
    ...
    for (const error of scenarioValidation.errors) errors.push(`Compiled scenario: ${error}`);
  } else {
...
function validateUniqueId(id: string, seenIds: Set<string>, errors: string[]): void {
  if (seenIds.has(id)) errors.push(`Duplicate flow node/action id '${id}'.`);
  seenIds.add(id);
}

function validateAction(node: FlowActionNode, catalogs: Catalogs, errors: string[]): void {
...
function validateEdges(flow: FlowFile, nodeIds: Set<string>, errors: string[], warnings: string[]): void {
  if (!flow.edges?.length) return;
  for (const edge of flow.edges) {
    if (!nodeIds.has(edge.from)) errors.push(`Flow edge references unknown source node '${edge.from}'.`);
    if (!nodeIds.has(edge.to)) errors.push(`Flow edge references unknown target node '${edge.to}'.`);
  }
...
function validateControlNode(node: FlowNode, catalogs: Catalogs, errors: string[], warnings: string[], seenIds: Set<string>, loopAncestors: FlowNode[] = []): void {
  if (node.type === "parallel") {
    if (node.branches.length === 0) errors.push(`Parallel node '${node.id}' must contain at least one branch.`);
    for (const branch of node.branches) {
      if (branch.nodes.length === 0) warnings.push(`Parallel node '${node.id}' branch '${branch.id}' has no steps.`);
      for (const child of branch.nodes) {
        validateUniqueId(child.id, seenIds, errors);
        if (child.type === "parallel" || child.type === "loop") validateControlNode(child, catalogs, errors, warnings, seenIds, loopAncestors);
        else if (child.type === "api_operation") {
          if (!catalogs.apiOperations[child.operation]) errors.push(`Flow node '${child.id}' references unknown API operation '${child.operation}'.`);
        } else validateAction(child, catalogs, errors);
      }
    }
  }
  if (node.type === "loop") {
    ...
    if (node.nodes.length === 0) errors.push(`Loop node '${node.id}' must contain at least one child step.`);
    if (node.loop.mode === "count" && node.loop.count === undefined) errors.push(`Loop node '${node.id}' count mode requires count.`);
    if (node.loop.mode === "foreach" && node.loop.items === undefined) errors.push(`Loop node '${node.id}' foreach mode requires items.`);
    for (const child of node.nodes) {
      validateUniqueId(child.id, seenIds, errors);
      if (child.type === "parallel" || child.type === "loop") validateControlNode(child, catalogs, errors, warnings, seenIds, [...loopAncestors, node]);
      else if (child.type === "api_operation") {
        if (!catalogs.apiOperations[child.operation]) errors.push(`Flow node '${child.id}' references unknown API operation '${child.operation}'.`);
      } else validateAction(child, catalogs, errors);
    }
...
function loopDesignerName(node: FlowNode): string {
...
function loopSummary(node: FlowNode): string {
  if (node.type !== "loop") return node.label ?? node.id;
  if (node.loop.mode === "foreach") return `foreach ${String(node.loop.items ?? "")}`.trim();
  return `count ${String(node.loop.count ?? 0)}`;
}

function firstVisibleLoopChildName(node: FlowNode): string | undefined {
...
function validateReferences(flow: FlowFile, catalogs: Catalogs, errors: string[], warnings: string[]): void {
  ...
  for (const node of ordered) {
    ...
    if (node.type === "api_operation") {
      for (const action of node.postActions ?? []) outputsByNode.set(action.id, outputNamesFor(action, catalogs));
    }
  }
...
function parseLoopReference(ref: string): { loopId: string; outputName: string } | undefined {
  const indexed = ref.match(/^([A-Za-z0-9_-]+)\[\d+\]\.(.+)$/);
  if (indexed) return { loopId: indexed[1], outputName: indexed[2] };
  const dot = ref.indexOf(".");
  if (dot <= 0) return undefined;
  return { loopId: ref.slice(0, dot), outputName: ref.slice(dot + 1) };
}

function isKnownLoopOutput(loopNode: FlowNode, outputName: string, catalogs: Catalogs): boolean {
  ...
  if (dateOutput) known.add(dateOutput);
...
function allFlowNodes(nodes: FlowNode[]): FlowNode[] {
  ...
  for (const node of nodes) {
    result.push(node);
    if (node.type === "api_operation") result.push(...(node.postActions ?? []));
    if (node.type === "parallel") {
      for (const branch of node.branches) result.push(...allFlowNodes(branch.nodes));
    }
    if (node.type === "loop") result.push(...allFlowNodes(node.nodes));
  }
  return result;
}

function outputNamesFor(node: FlowNode | FlowActionNode, catalogs: Catalogs): string[] {
  if (node.type === "api_operation") return uniqueStrings([...Object.keys(catalogs.apiOperations[node.operation]?.captures ?? {}), ...Object.keys(node.capture ?? {})]);
  if (node.type === "db_query" || node.type === "db_assert" || node.type === "db_execute") return uniqueStrings([...Object.keys(catalogs.queries[node.query]?.captures ?? {}), ...Object.keys(node.capture ?? {})]);
  if (node.type === "unix_batch") return uniqueStrings([...Object.keys(catalogs.batches[node.batch]?.captures ?? {}), ...Object.keys(node.capture ?? {})]);
  if (node.type === "loop") return uniqueStrings([loopDateOutputName(node) ?? "", ...Object.keys(node.capture ?? {})]);
  if (node.type === "parallel") return uniqueStrings([...Object.keys(node.capture ?? {})]);
  return [];
}
...
function nearestSuggestion(value: string, candidates: string[]): string {
...
function referencePayload(node: FlowNode | FlowActionNode): unknown {
  if (node.type !== "api_operation") return node;
  const { postActions: _postActions, ...payload } = node;
  return payload;
}

function validateRequiredInputs(node: FlowNode | FlowActionNode, catalogs: Catalogs, errors: string[], warnings: string[]): void {
  ...
  if (node.type === "db_query" || node.type === "db_assert" || node.type === "db_execute") {
    validateParams(
      node.id,
      normalizeBindParamRecord(params),
      normalizeBindParamRecord(catalogs.queries[node.query]?.params ?? {}),
      errors,
      "query param"
    );
  }
...
function validateParams(nodeId: string, params: Record<string, unknown>, specs: Record<string, CatalogParam> | undefined, errors: string[], label: string): void {
...
function validateArgs(nodeId: string, params: Record<string, unknown>, args: CatalogArg[], errors: string[]): void {
  for (const arg of args) {
    if (!Object.prototype.hasOwnProperty.call(params, arg.name)) continue;
...
function validateBatchInputFiles(nodeId: string, params: Record<string, unknown>, batch: Catalogs["batches"][string] | undefined, errors: string[]): void {
...
function publishExpectedOutputs(node: FlowNode | FlowActionNode, catalogs: Catalogs, availableBare: Set<string>, _warnings: string[]): void {
  if (node.type === "api_operation") {
    for (const key of Object.keys(catalogs.apiOperations[node.operation]?.captures ?? {})) availableBare.add(key);
  }
  if (node.type === "db_query" || node.type === "db_assert" || node.type === "db_execute") {
    for (const key of Object.keys(catalogs.queries[node.query]?.captures ?? {})) availableBare.add(key);
  }
  if (node.type === "unix_batch") {
    for (const key of Object.keys(catalogs.batches[node.batch]?.captures ?? {})) availableBare.add(key);
  }
  if (node.type === "loop") {
    const outputName = loopDateOutputName(node);
    if (outputName) availableBare.add(outputName);
  }
  for (const key of Object.keys(node.capture ?? {})) availableBare.add(key);
}

function loopDateOutputName(node: FlowNode | FlowActionNode): string | undefined {
...
function findRefs(value: unknown): string[] {
  ...
  const visit = (current: unknown) => {
    if (typeof current === "string") {
      for (const match of current.matchAll(/\$\{([^}]+)\}/g)) refs.push(match[1]);
...
function validateTemplateBinding(node: FlowNode | FlowActionNode, catalogs: Catalogs, warnings: string[]): void {
  if (node.type !== "api_operation" || !node.request) return;
...
function validateApiRequestBody(node: FlowNode | FlowActionNode, errors: string[]): void {
  if (node.type !== "api_operation" || node.request?.bodyMode !== "json" || node.request.rawBody === undefined) return;
...
function validateApiStepIntent(node: FlowNode | FlowActionNode, warnings: string[]): void {
  if (node.type !== "api_operation") return;
